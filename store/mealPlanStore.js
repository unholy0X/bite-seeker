...
export const useMealPlanStore = create((set, get) => ({
  ...
  loadWeek: async ({ getToken, weekOffset }) => {
    ...
    try {
      ...
      set({ error: err?.message || i18n.t("errors:mealPlan.loadFailed"), isLoading: false });
    }
  },
  ...
  navigateWeek: async ({ getToken, direction }) => {
    ...
    try {
      ...
      // Keep the local state with empty entries but surface the error
      set({ isLoading: false, error: err?.message || i18n.t("errors:mealPlan.loadFailed") });
    }
  },
  ...
  addEntry: async ({ getToken, recipeId, dayIndex, mealType }) => {
    ...
    if (!plan) return;
    if (!plan.id) {
      const err = new Error(i18n.t("errors:mealPlan.notSynced"));
    ...
    try {
      ...
      set({ error: err?.message || i18n.t("errors:mealPlan.addFailed") });
      throw err;
    }
  },
  ...
  removeEntry: async ({ getToken, entryId }) => {
    ...
    try {
      ...
      // Revert
      set({ plan: { ...plan, entries: prevEntries }, error: err?.message || i18n.t("errors:mealPlan.removeEntryFailed") });
      throw err;
    }
  },
  ...
  generateShoppingList: async ({ getToken, name }) => {
    ...
    try {
      ...
      set({ error: err?.message || i18n.t("errors:shopping.generateFailed"), isGenerating: false });
      throw err;
    }
  },
