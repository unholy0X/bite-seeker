...
export const useShoppingStore = create((set, get) => ({
    ...
    deleteLists: async ({ getToken, listIds }) => {
        ...
        if (failedIds.length > 0) {
            // Restore only the ones that failed, avoiding duplicates by
            // merging back into current state using the pre-delete snapshot
            // as the source of truth for failed items.
            const failedLists = prevLists.filter((l) => failedIds.includes(l.id));
            const failedSet = new Set(failedIds);
            set((state) => ({
                lists: [
                    ...state.lists.filter((l) => !failedSet.has(l.id)),
                    ...failedLists,
                ],
