...
export default function RecipiesScreen() {
          ...
          onPress: async () => {
            ...
            try {
              const results = await Promise.allSettled(
                [...selectedIds].map((id) => deleteRecipe({ recipeId: id }))
              );
  ...
  }, [router, handleToggleFavorite, handleLongPress, isSelectMode, selectedIds, toggleSelection, t]);

  const ListHeader = isSelectMode ? (
    <View style={styles.selectHeader}>
      {/* Row 1: Cancel | count | Select All */}
