...
export default function HomeScreen() {
  ...
  const handlePantryMatch = useCallback(() => {
    ...
    const promises = [loadPantry({})];
    if (allRecipes.length === 0) promises.push(loadAll());

    Promise.all(promises)
      .catch(() => {
        setSaveError(t("errors:home.pantryMatchFailed", "Couldn't load matches. Check your connection."));
  ...
  const handleSaveAndFavorite = useCallback(async (recipeId) => {
    ...
    try {
      const cloned = await cloneRecipe({ recipeId });
      const clonedId = cloned?.id || cloned?.recipe?.id;
      if (clonedId) {
        await toggleFavoriteApi({ recipeId: clonedId, isFavorite: true });
      }
      loadRecipes({}).catch(() => { });
    } catch (err) {
          ...
          ({ onScroll, scrollEnabled }) => (
            <>
              ...
              <RecipeGrid
                ...
                savingIds={savingIds}
                onSave={(id) => toggleFavorite({ recipeId: id }).catch(() => { })}
                onPressRecipe={(recipe) => {
                  setFavoritesOpen(false);
                  router.push(`/recipe/${recipe.id}`);
                }}
