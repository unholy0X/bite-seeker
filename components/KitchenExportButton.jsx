...
export default function KitchenExportButton({ recipeId, t, FONT, initialUrl, onSuccess }) {
  ...
  const runExport = useCallback(async (force = false) => {
    ...
    try {
      const result = await exportToKitchen({ recipeId, force });

      const jobId = result.jobId ?? result.jobID ?? result.id;
      if (jobId) {
        // Backend queued a new job — poll regardless of current status
        useJobsStore.getState().addActiveJob({ id: jobId, status: "pending", jobType: "thermomix_export" });
        startPolling(jobId);
  ...
  }, [recipeId, startPolling, onSuccess]);
