...
export default function NotificationCenter() {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      const data = await authFetch("/jobs?limit=20");
      ...
      for (const job of list) {
        ...
        } else if (isTerminal && !prev && !notifiedRef.current.has(id)) {
          // Discovery path: job completed while we weren't polling (polling gap) OR
          // first-fetch cold-start recovery. Both cases: never seen this job before.
