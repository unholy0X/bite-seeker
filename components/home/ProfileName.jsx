...
function WalletAvatar({ walletAddress, size = 50 }) {
  const name = walletAddress || "anon";
...
export default function ProfileName({ subtitle = "Your kitchen awaits", rightSlot }) {
  ...
  const displayName = truncateWallet(walletAddress);

  const toggleUnit = () => {
    const next = preferredUnitSystem === "metric" ? "imperial" : "metric";
    updatePreferences({ preferredUnitSystem: next });
  };
    ...
    <View style={styles.wrapper}>
      <View style={styles.welcomeCard}>
        <Pressable style={styles.welcomeLeft} onPress={() => setOpen(true)}>
          <WalletAvatar walletAddress={walletAddress} size={sc(50)} />
        ...
        <View style={styles.menuSheet}>
          <View style={[styles.sheetInner, { paddingBottom: insets.bottom + 20 }]}>
            ...
            <View style={styles.profileSection}>
              <WalletAvatar walletAddress={walletAddress} size={sc(56)} />
              <View style={styles.profileInfo}>
                <Text style={styles.profileName}>{displayName}</Text>
                {walletAddress ? (
                  <Text style={styles.profileEmail}>{walletAddress}</Text>
                ) : null}
              </View>
            </View>
            ...
            <Pressable style={styles.menuRow} onPress={toggleUnit}>
              ...
              <View style={styles.unitToggle}>
                <Text style={[styles.unitOption, preferredUnitSystem === "metric" && styles.unitOptionActive]}>
                  {t("profile.metric")}
                </Text>
                <Text style={[styles.unitOption, preferredUnitSystem === "imperial" && styles.unitOptionActive]}>
                  {t("profile.imperial")}
                </Text>
              </View>
            </Pressable>
              ...
              <View style={styles.unitToggle}>
                {["en", "fr", "ar"].map((lang) => (
                  <Pressable key={lang} onPress={() => setLanguage(lang)}>
                    <Text style={[styles.unitOption, language === lang && styles.unitOptionActive]}>
                      {lang.toUpperCase()}
                    </Text>
                  </Pressable>
                ))}
            ...
            {/* Subscription */}
            {entitlement === "pro" || entitlement === "admin" ? (
